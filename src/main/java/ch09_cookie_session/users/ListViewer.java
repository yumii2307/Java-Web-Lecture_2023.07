...
public class ListViewer extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (sessionUid != null) {				// 로그인 상태
			...
			html += " <button onclick=\"location.href='/jw/ch09/users/register.html'\">회원 가입</button>";
		}
		html += "<hr>"
				+ "    <table border=\"1\">"
				+ "        <tr>"
				+ "            <th>UID</th><th>PWD</th><th>Name</th><th>e-mail</th><th>등록일</th><th>액션</th>"
				+ "        </tr>";
		for (User u: list) {
			html += "<tr>"
					+ " <td>" + u.getUid() + "</td>"
